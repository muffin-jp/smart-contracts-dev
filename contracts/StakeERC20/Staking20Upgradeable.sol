...
abstract contract Staking20Upgradeable is ReentrancyGuardUpgradeable, IStaking20 {
     ...
     */
    function claimRewardsWithAmount(uint256 _amount) external nonReentrant {
    ...
    function getNextConditionId() public view returns (uint64 _conditionId) {
        _conditionId = nextConditionId;
    }

    function getStakingCondition(uint256 conditionId) public view returns (StakingCondition memory _stakingCondtion) {
        _stakingCondtion = stakingConditions[conditionId];
    }
    ...
    /// @dev Logic for claiming rewards. Override to add custom logic.
    function _claimRewards(uint256 amount) internal virtual {
        uint256 rewards = stakers[_stakeMsgSender()].unclaimedRewards + _calculateRewards(_stakeMsgSender());

        require(amount != 0, "Claiming 0 tokens");
        require(rewards != 0, "No rewards");
        require(amount <= rewards, "Amount exceeds rewards");

        stakers[_stakeMsgSender()].timeOfLastUpdate = uint80(block.timestamp);
        stakers[_stakeMsgSender()].unclaimedRewards = rewards - amount;
        stakers[_stakeMsgSender()].conditionIdOflastUpdate = nextConditionId - 1;

        _mintRewards(_stakeMsgSender(), amount);

        emit RewardsClaimed(_stakeMsgSender(), amount);
    }
